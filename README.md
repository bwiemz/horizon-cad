...
| 155 | Done | Variables: Edit ▸ Variables lists a document's named values ("wall" = "3 mm", "width" = "10 * wall"), each worked out as it is typed and what it measures checked (a length is not added to a number; units in expressions: mm, cm, m, in, ft, deg, rad); names, loops and errors refused with why; one undo step. a feature's length or angle may be "=expression" of them in Edit Feature ("=wall * 2"), kept with its units written ("wall + 1" typed in inches is kept as "wall + 1 in"), worked out at each build (a worker's too), a failure that feature's, saved beside the value an older build reads |
| 156 | Done | Configurations: a part's design table (Edit ▸ Configurations: a row for each configuration, a cell for each variable it gives another expression, blank for the variable's own), saved; the part built in the configuration chosen above the feature tree, its constraints solved with it too; the variables themselves never written, so none chosen is the part as it was; each choice one undo step |
| 157 | Done | Sketches that follow: a sketch made on a part's face follows it by name when the part changes (a taller box carries the sketch on its top, and the boss drawn there, up with it), an extrusion's direction and a revolve's axis taken along; a face gone or no longer flat fails the feature, saying which sketch and face. Part edges projected into a sketch (Model ▸ Project Edges: a line, a circle or arc square on, else a polyline), following their edges as the part changes and held by the constraint solver; construction geometry (Model ▸ Construction), dashed and never part of a profile. Extrude up to a flat face parallel to the sketch, wherever the part puts it. Milestone 16 complete |
| 158 | Done | Drag with the mates: a component dragged in the view goes where the cursor takes it, its mates solved as it moves (held where it is put; where they cannot follow, it slides as far as they let it); the chosen component's triad moves it along one of its axes or turns it about one; a component held by a Fixed mate is not dragged; one undo step, Escape puts it back |
| 159 | Done | Subassemblies: an assembly inserted into another as a component, shown whole, its components resolved from its file recursively (an assembly inside itself, at any depth, refused and the chain named); its faces mated in the parent; interference, drawings and STEP export (its parts placed in one level) take it; the tree lists its components under it; the bill of materials top level, indented (2.1, 2.2...) or parts only; a part changed deep inside, or the subassembly saved, refreshes every assembly it is in |
| 160 | Done | More to mate: mates on straight and round edges, datum planes, axes and points, spheres and cones as well as faces (Coincident, Concentric, Distance, Angle, Parallel, Perpendicular and Tangent each for the pairs it relates; others refused with why); Distance and Angle mates between limits, free within them; an angle held at 0 or 180 degrees; a frustum's ideal cone right, so it goes to STEP as designed; an unknown mate type in a file skipped and said |
| 161 | Done | Exploded views and component patterns: named exploded views, each a list of steps (components moved together along a direction by a distance), shown or not without ever moving where a component is placed, its mates or its file; linear and circular patterns of components whose instances are components of their own, placed from their seeds and following them, counted in the bill of materials and checked for interference, one left out by removing it; mirrored components moved to Phase 162, with the mirrored body |
| 162 | Done | Hole and Mirror: a Hole feature (simple, counterbored or countersunk; to a depth with a drill point or flat, through all, or up to a parallel face) drilled into a flat face at a point and following it, its walls true cylinders and cones that go to STEP as designed; a Mirror feature of the whole part or of chosen features, in a base plane or a flat face it follows, the mirror image facing out as the original does (the kernel reverses a mirrored copy's loops and surfaces); mirrored components in an assembly, their parts mirrored and placed rigidly, a line of their own in the bill of materials and a part of their own in STEP |
| 163 | Done | Shell, part 2: a shell made by moving each face (a plane along its normal, a cylinder, cone or sphere grown or shrunk) and cutting the cavity from the part, so any single body of flat, cylindrical, conical and spherical faces is hollowed, with holes, bosses and as many open faces as are chosen; the part keeps its faces, names and true surfaces, the cavity's on the offset ones; a corner whose faces do not offset to one point, or too thick a wall, refused and said; a shell from an older file built as it was |
| 164 | Done | Fillets on curved faces: a revolve's rim filleted (a solid of revolution's, a ring's outer rim), its flat end one face with its hole kept, and Fillet keeping every face's holes; a rim's fillet exact at each corner (the true section in the meridian plane), so its bands are inscribed in the design's torus, which they record as their one ideal, with circles on its tangent lines, and the part measures as designed; a Boolean cut of a face with a hole kept off round loops' chords; STEP export of a filleted rim still in facets, said |
| 165 | Done | Booleans at scale, part 1: a convex operand's tree (a pin, a cylinder of thousands of facets) built in one pass, the same tree the split-by-split build made, so results are unchanged: a box less a 2,048-facet pin 0.81 s to 0.3 s optimised (4.0 s to 1.7 s in a Debug build); clipping only what is near, for a many-holed plate drilled again, left for a new Boolean core |
| 166a | Done | Every string translated: the whole interface (1,107 strings) in German, Spanish, French, Japanese, Korean and Chinese, where 18 were before. They are machine translations, marked unreviewed in the catalogs for a native speaker to check. The 36 main-window labels whose context was wrong (so never translated) are fixed. A catalog check runs as a test in every build: each language at least 95% translated, placeholders and mnemonics as in the English, and no string in the sources missing from the catalogs |
| 166b | Done | Accessible names: every control a screen reader reaches has a name, translated with the rest of the window. The trees and lists (features, layers, components and mates, constraints, sketches), the configuration choice, the colour button and the viewport are named in code, and each document tab's close button says which document it closes. A form's fields take their labels' names. A test walks the window and a form through Qt's accessibility interface and fails on any unnamed control |
| 167 | Done | Crash reports: a crash in native code (a bad memory access, an abort, a stack overflow; on Windows an unhandled exception, with a minidump) leaves a report of what happened (the signal, the address, a backtrace, the version and platform), written only with calls safe in a signal handler, on a stack of its own. At the next start it is offered, with the log's last lines, to save and attach to an issue, or delete; nothing is sent anywhere. The log is flushed line by line so it holds what led up to a crash. Tested by a helper process that crashes three ways, and in CI by crashing the application itself |
| 168a | Done | A user guide in the application: Help ▸ User Guide (F1) opens it at the page for what is being worked on. Ten pages written from the code (getting started, 2D drafting, sketches and constraints, parts, assemblies, drawing sheets, files and recovery, settings, keyboard shortcuts), compiled in as Markdown and shown by Qt, so it needs no browser and works offline. Tests check every link and page, and make the guide's first part as it says. Writing it found Polyline Edit's A, D and C keys switching tools, fixed |
