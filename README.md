...
| modeling — extrude/revolve/primitives/patterns | stable | Exact volumes verified. Curved primitives (cylinder/cone/sphere/torus) are faceted at construction — `segments` is tunable, `segmentsForTolerance()` derives it from a chord-sag budget — so their volumes converge to the analytic value rather than matching it exactly; each facet records the analytic surface it approximates |
