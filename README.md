...
| fileio — drawings (`.hzdwg`, drawing DXF) | experimental | Drawing ▸ New Drawing from Part makes a sheet tab: the standard views at an ISO scale, with border and title block, saved as `.hzdwg`, drawn again when the part changes, exported to DXF, PDF and SVG at 1:1. Views are placed by the layout: moving one, sections and details are not in the application yet |
