...
| 63 | Done | Era 3 — Python API (phase 2): sheet-metal, and FEA static + modal analysis on the current solid (`doc.static_analysis`, `doc.modal_analysis`), bound into the embedded `horizon` module |
...
| 73 | Done | Era 4 — Advanced simulation (modal analysis): consistent-mass linear-tet element + generalized eigensolve `K φ = ω² M φ` for natural frequencies and mode shapes; validated by exact scaling invariances (f ∝ √E, 1/√ρ, 1/size), free-free rigid-body modes, and ordering; exposed as `doc.modal_analysis` |
