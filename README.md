...
| 158 | In progress | Drag with the mates: a component dragged in the view goes where the cursor takes it, its mates solved as it moves (held where it is put; where they cannot follow, it slides as far as they let it); a component held by a Fixed mate is not dragged; one undo step, Escape puts it back. The triad is next |
