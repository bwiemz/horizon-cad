...
| document / undo | stable | Feature tree + multi-document are newer but well-tested. Faceting resolution is a parameter of the feature that owns it (`segments` on curved primitives and revolves, `arcSegments` on fillets), so accuracy is editable and persists across save/reload |
