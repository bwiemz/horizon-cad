...
Arc2D::Arc2D(const math::Vec2& center, double radius, double startAngle, double endAngle)
    : m_center(center), m_radius(radius), m_startAngle(startAngle), m_endAngle(endAngle) {}
...
math::Vec2 Arc2D::evaluate(double t) const {
    ...
    return math::Vec2(m_center.x + m_radius * std::cos(angle),
...
math::Vec2 Arc2D::derivative(double t, int order) const {
    ...
    if (order == 1) {
        ...
        return math::Vec2(-m_radius * std::sin(angle) * dAngle,
                          m_radius * std::cos(angle) * dAngle);
    }
    if (order == 2) {
        double dAngle2 = dAngle * dAngle;
        return math::Vec2(-m_radius * std::cos(angle) * dAngle2,
