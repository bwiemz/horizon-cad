...
class TypedPoint {
public:
    /// Handle a key (a Qt::Key): a digit, `.`, `-`, `,`, `@`, `<`, or
    ...
    /// point the cursor shows (@p toward). The point; or nothing, with the
    /// text kept as refused and @p why saying why.
    std::optional<math::Vec2> take(const std::optional<math::Vec2>& base,
                                   const std::optional<math::Vec2>& toward);

    /// For the prompt, with a leading gap: what is typed ("  Point: @5<30"),
    /// else the last refusal ("  '5,,3' is not a point: ..."), else "".
